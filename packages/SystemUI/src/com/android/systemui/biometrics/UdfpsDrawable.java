...
public abstract class UdfpsDrawable extends Drawable {
    ...
    String udfpsResourcesPackage = "com.arrow.udfps.icons";
...
/*    void init() {
        if (systemUtils.isPackageInstalled(mContext, udfpsResourcesPackage)) {
            try {
...
*/    
    ...
    void init() {
        ...
        if (ArrowUtils.isPackageInstalled(mContext, udfpsResourcesPackage)) {
            try {
                PackageManager pm = mContext.getPackageManager();
                udfpsRes = pm.getResourcesForApplication(udfpsResourcesPackage);
            } catch (PackageManager.NameNotFoundException e) {
